...
(function() {
   ...
   function isTargetComponent(componentClassName) { 
    ...
    function getDataURL() {
        ...
        console.log(endpoint + "?query=" + queryParam + "&limit=" + limit);
    ...
    function displayDataOnPage(resultData) {
        ...
        for (var i = 0; i < dataStoreCount; i++) {
            LIST_GROUP += "<li class='cmp-searchresult-item'><a class='cmp-searchresult-link' target='_blank' href=" + checkNull(dataStore[i].uri) + "><img src = "+checkNull(dataStore[i].featuredImage)+"> <p><img src = "+checkNull(dataStore[i].brandImage)+"></p></a></li>";
        }

        for (var i = 0; i < dataArticleCount; i++) {
            LIST_GROUP_Article += "<li class='cmp-searchresult-item'><a class='cmp-searchresult-link' target='_blank' href=" + checkNull(dataArticle[i].uri) + "><img src = "+checkNull(dataArticle[i].featuredImage)+"><span>" + checkNull(dataArticle[i].publishedDate) + "</span><h3 class='cmp-searchresult-title'>" + checkNull(dataArticle[i].title) + "</h3><p>" + checkNull(dataArticle[i].description) + "</p></a></li>";
        }
