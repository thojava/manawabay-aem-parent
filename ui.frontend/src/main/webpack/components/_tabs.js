    ...
    function Tabs(config) {
        ...
        function scrollToDeepLinkIdInTabs() {
            if (containerUtils) {
                ...
                const deepLinkItems = entries.filter(entry => entry.includes(that._elements.self.id));

                if (deepLinkItems) {
                    deepLinkItems.forEach(item => {
                        const deepLinkItem = that._elements["tab"].find(element => element.id === item);
                        const deepLinkItemIdx = that._elements["tab"].indexOf(deepLinkItem);
        ...
        function updateUrlHash(index) {
            ...
            const item = that._elements["tab"][index];
            const entries = hash.substring(1).split('&');
            const filteredHash = entries.filter(entry => !entry.includes(that._elements.self.id));
            const newHash = filteredHash.join('&');

            if (newHash) {
                location.hash = `${newHash}&${item.id}`;
            } else {
